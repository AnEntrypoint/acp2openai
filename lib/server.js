...
async function listModels() {
  ...
  for (const r of results) {
    if (!r.ok) continue;
    data.push({ id: `${r.prefix}/${r.default}`, object: 'model', owned_by: r.prefix, created: Math.floor(Date.now()/1000) });
    if (r.prefix === 'kilo') ['kilo-auto/free', 'openrouter/free', 'x-ai/grok-code-fast-1:optimized:free'].forEach(m => data.push({ id: `kilo/${m}`, object: 'model', owned_by: 'kilo', created: Math.floor(Date.now()/1000) }));
    if (r.prefix === 'opencode') ['minimax-m2.5-free', 'nemotron-3-super-free'].forEach(m => data.push({ id: `opencode/${m}`, object: 'model', owned_by: 'opencode', created: Math.floor(Date.now()/1000) }));
  }
