...
const SSE_HANDLERS = {
  ...
  'reasoning-delta': (ev, state) => {
    const chunk = {id: state.id, object:'chat.completion.chunk', created: state.created, model:'', choices:[{index:0, delta:{reasoning_content: ev.reasoningDelta}, finish_reason:null}]};
    return 'data: ' + JSON.stringify(chunk) + '\n\n';
  },
